...
- **MCP registration** must include `--expected-sha256=<hash>` so the launcher can self-verify on every Claude Code session. See [ADR-0001](../../docs/decisions/0001-launcher-byte-immutability-and-sha-pin.md) for why the launcher template is byte-immutable and what threats this defends against (and which it explicitly does not).
