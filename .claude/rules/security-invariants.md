...
- **Repo deletion** (whether via `gh repo delete` shorthand or `gh api --method DELETE /repos/<slug>`) must be preceded by an explicit ownership check (`isAdmin` from [src/lib/github-repo.ts](../../src/lib/github-repo.ts)) and a typed-name confirmation. The argv shape changed in 2.7.1 (migrated to `gh api` for header-aware retry); the precondition is unchanged. `deleteRepo` is the single source of truth for the call shape.
