...
- **MCP registration** must include `--expected-sha256=<hash>` so the launcher can self-verify on every Claude Code session. See [ADR-0001](../../docs/decisions/0001-launcher-byte-immutability-and-sha-pin.md) for why the launcher template is byte-immutable and what threats this defends against (and which it explicitly does not). Same shape applies to the **search launcher** (`lib/search-launcher.js.tmpl`, installed at `~/.vaultkit/search-launcher.js` and registered as the global `vaultkit-search` MCP) — see [ADR-0010](../../docs/decisions/0010-bm25-search-mcp.md). Both launchers share `runMcpAdd` from `src/lib/mcp.ts` so the pin invariant is structurally impossible to omit on either path.
