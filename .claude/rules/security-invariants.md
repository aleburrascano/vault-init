...
- **MCP registration** must include `--expected-sha256=<hash>` so the launcher can self-verify on every Claude Code session. See [ADR-0001](../../docs/decisions/0001-launcher-byte-immutability-and-sha-pin.md) for why the launcher template is byte-immutable and what threats this defends against (and which it explicitly does not). The invariant applies only to the per-vault launcher (`lib/mcp-start.js.tmpl`); the previously-shipped global search launcher was retired in [ADR-0011](../../docs/decisions/0011-vaultkit-mcp-replaces-obsidian-mcp-pro.md) when search was folded into the per-vault MCP server. `runMcpAdd` from `src/lib/mcp.ts` remains the single source of truth for the registration argv shape.
- **Repo deletion** (whether via `gh repo delete` shorthand or `gh api --method DELETE /repos/<slug>`) must be preceded by an explicit ownership check (`isAdmin` from [src/lib/github-repo.ts](../../src/lib/github-repo.ts)) and a typed-name confirmation. The argv shape changed in 2.7.1 (migrated to `gh api` for header-aware retry — see [ADR-0004](../../docs/decisions/0004-deleteRepo-via-gh-api-for-header-aware-retry.md)); the precondition is unchanged. `deleteRepo` is the single source of truth for the call shape.
...
- **External-CLI Anti-Corruption Layer must not be bypassed.** All `gh` invocations from any file must go through one of the three github-* wrappers ([github-repo.ts](../../src/lib/github-repo.ts) for `createRepo` / `deleteRepo` / `setRepoVisibility` / `isAdmin` / …; [github-pages.ts](../../src/lib/github-pages.ts) for `enablePages` / `setPagesVisibility` / …; [github-auth.ts](../../src/lib/github-auth.ts) for `getCurrentUser` / `requireAuthGatedEligible` / `ensureDeleteRepoScope` / …) or `ghJson` / `gh` from [src/lib/gh-retry.ts](../../src/lib/gh-retry.ts) — never raw `execa('gh', …)` (or `execa(ghPath, …)`). Same rule for `claude mcp <verb>` (must go through `runMcpAdd` / `runMcpRemove` / `runMcpRepin` from [src/lib/mcp.ts](../../src/lib/mcp.ts)) and for `git` mutations that retry on transient / abuse-flag stderr (must go through [src/lib/git.ts](../../src/lib/git.ts)'s `pushNewRepo` / `pushOrPr`). Bypassing these boundaries loses (a) the retry / rate-limit handling that prevents account flagging on burst, (b) the `auth_flagged` stderr classification that maps to `AUTH_REQUIRED` exit code 7 instead of an unclassified exit 1, (c) the `--expected-sha256=<hash>` launcher-pin invariant on MCP registration, and (d) the poll-after-mutate that prevents stale-read races. See [.claude/rules/architecture.md](architecture.md) for the ACL boundary description.
