...
**Runtime**: Node.js ≥22.13, ESM (`"type": "module"`), TypeScript source under [bin/](../../bin/), [src/](../../src/), [tests/](../../tests/). The `≥22.13` floor (vs. plain `≥22`) is required for `node:sqlite` to be available unflagged — see ADR-0011 / `src/lib/search-index.ts`.
...
| `search-index.ts` | SQLite FTS5 + BM25 search index for vaultkit-managed vaults, backed by Node's built-in `node:sqlite` (Node ≥22.13 — zero npm dependency). Owns the schema (`notes(vault, path, title, tags, body)`), upsert/delete operations, and BM25-ranked queries with title 5x / tags 3x / body 1x weighting. Exports `SearchIndex` class (`upsert`, `delete`, `query`, `listVaults`, `listPaths`, `listTags`, `notesByTag`, `count`, `close`), `openSearchIndex(dbPath?)`, `defaultSearchDbPath()`, and `_sanitizeQuery` (test-only). User queries are OR-joined after stripping FTS5 operators so multi-token natural-language queries find any partial-match note (BM25 still ranks notes matching all terms highest). Lives at `~/.vaultkit-search.db` in WAL mode for concurrent access by the indexer (writer) and per-vault MCP server processes (readers). Used by `search-indexer.ts` (population), `src/commands/mcp-server.ts` and `src/mcp-tools/` (queries). See ADR-0010 for the BM25-vs-semantic decision (rationale still applies); ADR-0011 covers the engine swap from `better-sqlite3` to `node:sqlite` and the consolidation of search into the per-vault MCP. |
