...
- **Single shared fixture for `connect` / `disconnect` / `visibility` (2.7.3).** `tests/global-fixture.ts`'s vitest globalSetup creates one `vk-live-shared-${pid}-${ts}` repo at suite start and tears it down at suite end. The three fixture-sharing live blocks read its name via `tests/helpers/live-fixture.ts:getFixtureName()` and reset to baseline state in `beforeEach` (re-clone if disconnected; re-set visibility to private if drifted). The `init` and `destroy` live tests stay self-contained because they ARE the create / delete paths — coupling that coverage to fixture lifecycle would muddy what those tests validate.
