...
**Tests are always live.** As of v2.5.0 there is no `VAULTKIT_LIVE_TEST` env-gate — every `npm test` run hits the real GitHub API and creates ephemeral `vk-live-*` repos against the authenticated `gh` account. Files are run sequentially (`fileParallelism: false` in `vitest.config.ts`) to avoid `~/.claude.json` write races; see [ADR-0006](../../docs/decisions/0006-vitest-fileParallelism-disabled.md) for the trade-offs. CI rotates between two dedicated PATs (`VAULTKIT_TEST_GH_TOKEN_A` / `_B`) round-robin via `GITHUB_RUN_NUMBER % 2` — see `.github/workflows/main.yml`'s "Select test PAT" step. The `vaultkit-live-tests` concurrency group ensures only one CI run touches either account at a time.
...
- `tests/architecture.test.ts` — architectural fitness functions. Encodes the boundary rules from `.claude/rules/architecture.md`, `.claude/rules/security-invariants.md`, and `.claude/rules/code-style.md` as runnable checks: ACL non-bypass (no raw `execa('gh', …)` from commands; no `claude mcp <verb>` outside `mcp.ts`), ESM-only invariant (no `child_process` imports), `~/.claude.json` ownership (only `registry.ts` + `platform.ts` reference `claudeJsonPath`), and command module shape (every `src/commands/*.ts` exports `async function run(...)` and includes the `CommandModule` sentinel). When a check fails, the diff has crossed a documented boundary — fix the violation, grandfather it in the file's `EXCEPTIONS` map with rationale, or update the rule file if the boundary moved. The `EXCEPTIONS` list itself is the canonical record of known follow-ups; keep it small and pinned.
