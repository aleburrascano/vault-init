...
- **`vaultkit init` retries the initial `git push` on "Repository not found"** with the same backoff via a new `pushNewRepo` helper in [src/lib/git.ts](src/lib/git.ts). This is a separate retry surface because the failure is in the git push layer, not the gh API — GitHub has eventual consistency between `gh repo create` returning and the new repo's git endpoint accepting pushes. On fast machines, the push hits before propagation completes.
