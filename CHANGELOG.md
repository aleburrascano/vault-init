...
- **One-time post-upgrade migration notice (`src/lib/post-upgrade-check.ts`).** When vaultkit detects on the first command after an `npm update` that the running version differs from `~/.vaultkit-last-seen-version.json`'s value, it prints a single loud notice naming every registered vault whose launcher is now stale and surfaces the `vaultkit update --all` migration command. After the notice fires once, the cache is updated so subsequent invocations of the same version stay quiet. Solves the "user upgraded vaultkit through Renovate / Dependabot / `npm outdated` and has no idea their next Claude Code session will fail" problem at the highest-signal moment — the first CLI run after the upgrade. Best-effort throughout: a corrupt registry surfaces the version-bump line alone (with a `vaultkit doctor` pointer) but does not trap the user in an infinite-notice loop. Reuses the `VAULTKIT_NO_UPDATE_CHECK=1` env gate. Wired into `bin/vaultkit.ts:wrap()` after every successful action, alongside `checkForUpdate`.
