...
### Fixed
- **`vaultkit visibility <vault> public` no longer races against GitHub's Pages-auth cache.** `setRepoVisibility` returns 200 immediately, but downstream `/pages` endpoints can see the OLD private state — surfaced as a 422 "Your current plan does not support GitHub Pages for this repository" when `enablePages` was called next, even on accounts that genuinely have Pages access. Empirically the lag is >30s on Free-tier accounts, well past any reasonable retry budget. The fix is four layers, smallest to largest scope: (1) `setRepoVisibility` / `enablePages` / `setPagesVisibility` each poll their corresponding read endpoint (`getVisibility` / `pagesExist` / `getPagesVisibility`) post-mutation via the new `src/lib/poll.ts:pollUntil` helper, so the change is observable on the metadata path before the wrapper returns. (2) The Pages-plan 422 message is added to `gh-retry.ts:_classifyGhFailure`'s transient bucket as a retry-able backstop. (3) The transient retry schedule was extended from `[1s, 2s, 4s]` (7s total) to `[1s, 2s, 4s, 8s, 16s]` (31s total) so backstop retries cover most propagation windows. (4) **`vaultkit visibility` now graceful-fails Pages-related actions** (`enablePages` / `disablePages` / `setPagesVisibility`) — matching `init.ts:setupGitHubPages`'s existing pattern (`try` + `log.warn` + manual recovery URL). The visibility flip itself is the user's primary intent; Pages is secondary, and on Free-tier accounts the propagation window can outlast any retry budget. Aborting the whole command on a Pages 422 left the repo correctly flipped but the user staring at an error — better to log the gap and give them the recovery URL.

### Added
- **`src/lib/poll.ts`** — generic `pollUntil(read, predicate, opts?)` helper with `timeoutMs` (default 30s) and `intervalMs` (default 500ms). Throws `VaultkitError('NETWORK_TIMEOUT')` on timeout with the description and last-observed value embedded in the message. Distinct from `gh-retry.ts`'s retry-on-failure layer — `pollUntil` is for the "API call succeeded but its effect isn't visible yet" case. Used by the three Pages-related wrappers above. Captured as a project rule in `.claude/rules/code-style.md` ("Poll-after-mutate at eventual-consistency boundaries") so future wrappers around GitHub mutations follow the same pattern.
