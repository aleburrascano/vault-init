...
### Architecture
- **Git CLI Anti-Corruption Layer fitness function added.** Mirrors the gh-ACL check: no `src/commands/*.ts` file may invoke git via raw `execa('git', ...)`. The git ACL was always declared in `.claude/rules/architecture.md` and `security-invariants.md` but never enforced — six command files (`backup`, `doctor`, `init`, `status`, `update`, `verify`) had drifted into raw `execa('git', ...)` calls over time. They're grandfathered in `EXCEPTIONS['git-bypass-execa']` and removed one per commit (A3-A8). Also fixed a latent bug in the existing gh-ACL regex: a trailing `\b` after the literal-quote pattern never matched (quote characters aren't word characters), meaning the gh ACL would silently miss any future `execa('gh', ...)` regression. The git ACL inherited the same bug — both fixed in this commit.
