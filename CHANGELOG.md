...
- **`vaultkit setup` registers the search MCP automatically.** New step 6 in setup: idempotent — if `vaultkit-search` is already in `~/.claude.json#mcpServers`, repin (handles vaultkit upgrades that change the launcher template SHA); otherwise register fresh. Best-effort: failures log `! warn` but don't increment the issue count (search is value-add, not critical-path). Setup also pre-installs the launcher template even when the claude CLI is missing, so a future manual `claude mcp add` can pin against the same bytes. Added 4 mocked unit tests in a new `tests/commands/setup-search.test.ts` covering the register / repin / no-claude-skip / failure paths.
