...
- **Runtime dependencies dropped from `package.json`**: `better-sqlite3` (~12 MB native binary, replaced by `node:sqlite`), `@modelcontextprotocol/sdk` (~5.6 MB, replaced by hand-rolled JSON-RPC stdio in `src/lib/mcp-stdio.ts`), `zod` (~5.9 MB, replaced by hand-rolled validators in `src/lib/json-rpc-validator.ts`). Net 117 transitive packages removed from a fresh `npm install`. Plus `obsidian-mcp-pro` is no longer downloaded into the npx cache (~12 MB with deps). Combined: **~36 MB lighter on every user's machine** vs. the pre-rework baseline.
